...
/*		try {
	...
	public  List<JSONObject> helper(String search) throws ClientProtocolException, IOException {
		...
		buffer.append("http://api.bestbuy.com/v1/products");
		buffer.append("((");
		...
		buffer.append("))");
		buffer.append("?apiKey=5rrt9gjbgkutaz4wngznx76y&format=json&lsPublisherId=3266265");	
		//&pageSize=100
		...
		int count=0;
		if (!xmlJSONObj.isNull("products"))
		{
		    Object item = xmlJSONObj.get("products"); 
		    if (item instanceof JSONArray)
		    {
		    	JSONArray outputReturns = (JSONArray) item;
				  for (int i = 0; i < outputReturns.length(); i++) {
					    ...
					    if(!(outputReturn.isNull("regularPrice"))){
					    	msrp= outputReturn.get("regularPrice").toString();	
					    }
					    ...
					    if(!(outputReturn.isNull("image"))){
					    mediumImage= outputReturn.get("image").toString();
					    }
					    if(!(outputReturn.isNull("url"))){
					    productUrl= outputReturn.get("url").toString();	
					    }
